...
export async function POST(req) {
  ...
  Dimensions: improvement=${dims.pain || '?'}, data=${dims.data || '?'}, processDef=${dims.processDefinition || '?'}, integration=${dims.integration || '?'}, adoption=${dims.adoption || '?'}
  Gap: ${s.gap || 'Unknown'}
  ...
  Already automated: ${automatedActivities.map(a => a.name).join(', ') || 'minimal'}`;
    ...
    const result = await requestLlm({
      systemPrompt:
        'You are a senior automation activation planner specialising in enterprise process automation. You design rigorous, phased automation agent roadmaps grounded in real gap analysis. Return only valid JSON — no markdown, no explanation.',

      userPrompt: `Generate a 3-phase Automation Activation Plan for **${processName}**.
...
## PHASE GUIDANCE (from discovery analysis)
...
## AUTOMATION AGENT DESIGN RULES
- **Category "Native"**: automation agent uses built-in features of an existing platform (e.g. Salesforce Einstein, SAP workflow rules, Power Automate flow)
- **Category "Custom"**: automation agent requires custom development, API integration, or a new AI model
- **Effort** (choose based on activity type from the process discovery):
...
- **Impact**: tie directly to activities rated MANUAL or NOT_IN_PLACE in the process discovery
...
- Each automation agent must target a specific activity from the process discovery -- not generic descriptions
- Platform must reference a specific tool from the tech stack

## PHASE RULES
- Phase 1 (Weeks 1-6): 3-5 automation agents -- target PARTIAL activities where a small push achieves AUTOMATED; prefer Native automation agents on existing platforms
- Phase 2 (Weeks 7-16): 4-6 automation agents -- target MANUAL activities in core process steps; mix of Native and Custom
- Phase 3 (Weeks 17-24): 3-5 automation agents -- target NOT_IN_PLACE and deepest MANUAL gaps; advanced AI/ML, autonomous decisions
  ...
  "phases": [
    {
      ...
      "timeframe": "Weeks 1-6",
      "description": "<2 sentences describing the Phase 1 strategy and why these automation agents come first>",
      "agents": [
        {
          "id": "a1",
          ...
          "stageId": "<exact stageId from process discovery>",
    ...
    },
    {
      ...
      "subtitle": "Close the deepest gaps with new automation capability",
...
Valid stageIds: ${stages.map(s => s.stageId).join(', ')}`,
    ...
    // Compute totalAgents (automation agents) from actual phases
